...
export function writeSecretFile(filePath: string, content: string): void {
  try {
    ...
    console.warn(`[ElementKore-Wa] pre-write chmod 0o600 failed for ${filePath}: ${(error as Error).message}`);
  }
  writeFileSync(filePath, content, { mode: 0o600 });
  try {
    chmodSync(filePath, 0o600);
  } catch (error) {
    console.warn(`[ElementKore-Wa] post-write chmod 0o600 failed for ${filePath}: ${(error as Error).message}`);
  }
}
